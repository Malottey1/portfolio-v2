...
import { PLAYER_IDLE, PLAYER_WALK_1, PLAYER_WALK_2, TILE_GROUND, TILE_PLATFORM, drawGrid, drawTiled } from './sprites';
...
export function startGame(canvas: HTMLCanvasElement, debugEl: HTMLElement | null) {
	...
	function render() {
		...
		for (const p of platforms) {
			const tile = p.type === 'platform' ? TILE_PLATFORM : TILE_GROUND;
			drawTiled(ctx, tile, p.x, p.y, p.width, p.height);
		}
